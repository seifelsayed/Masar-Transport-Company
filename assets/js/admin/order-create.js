...
import { getMerchants, getZones, getOrders, saveOrders, getSettings, getLogo, getOrderCount, saveOrderCount } from '../storage.js';
...
if (checkAuth('admin')) {
  ...
  const logo = getLogo() || '../assets/logo.jpeg';

  let form = {
    merchantUsername: '', zone: '', zonePrice: 30,
  ...
  const handleSubmit = (e) => {
    ...
    const now = new Date();
    const selectedMerchant = merchants.find(m => m.username === form.merchantUsername);
    const selectedZone = zones.find(z => String(z.id) === String(form.zone));

    const order = {
      id: orderNum,
      merchant: selectedMerchant ? (selectedMerchant.company || selectedMerchant.name || selectedMerchant.username) : form.merchantUsername,
      customer: form.customerName,
      phone: form.customerPhone,
      address: form.customerAddress,
      zone: selectedZone ? selectedZone.name : form.zone,
  ...
  const render = () => {
      ...
      <div class="max-w-4xl mx-auto">
        <div class="flex items-center justify-between mb-8">
          <div>
            ...
            <p class="text-white/40 text-sm mt-1">أدخل بيانات الشحنة لإنشاء البوليصة</p>
          </div>
        ...
        <form id="order-form" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div class="bg-white/5 rounded-3xl p-6 border border-white/5 space-y-5">
            <h3 class="text-blue-400 text-xs font-bold uppercase tracking-widest mb-2">بيانات التاجر والمنطقة</h3>
            
            <div class="space-y-1.5">
              <label class="text-xs font-bold text-white/40">التاجر</label>
              <div class="relative">
                <select id="merchant-select" required class="w-full h-[46px] bg-[#13131f] border border-white/10 rounded-xl py-2.5 pr-4 pl-10 text-white text-sm outline-none focus:border-blue-500/50 appearance-none transition-all">
                  <option value="" class="bg-[#13131f] text-white">اختر تاجر...</option>
                  ${merchants.map(m => `<option value="${m.username}" ${form.merchantUsername === m.username ? 'selected' : ''} class="bg-[#13131f] text-white">${m.company || m.name || m.username}</option>`).join('')}
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="space-y-1.5">
                ...
                <input type="number" id="zone-price" value="${form.zonePrice}" class="w-full h-[46px] bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-white text-sm outline-none focus:border-blue-500/50">
              </div>
            </div>
          </div>
          ...
          <div class="lg:col-span-2 bg-white/5 rounded-3xl p-6 border border-white/5 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div class="space-y-1.5">
                <label class="text-xs font-bold text-white/40">المبلغ المطلوب تحصيله (COD)</label>
            ...
            <div class="space-y-1.5">
              ...
              <textarea id="notes" rows="2" class="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-white text-sm outline-none focus:border-blue-500/50">${form.notes}</textarea>
            </div>
          </div>
          ...
          <div class="lg:col-span-2">
            <button type="submit" class="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white font-black py-4 rounded-2xl text-lg shadow-xl shadow-blue-600/20 hover:opacity-90 transition-all flex items-center justify-center gap-3">
              <i class="bi bi-check-circle"></i> حفظ الطلب وإنشاء البوليصة
            </button>
          </div>
        </form>
      </div>
    ...
    document.getElementById('merchant-select').onchange = (e) => form.merchantUsername = e.target.value;
    document.getElementById('zone-select').onchange = (e) => {
      ...
      if (z) {
        form.zonePrice = z.price;
        document.getElementById('zone-price').value = z.price;
      }
    };
    document.getElementById('zone-price').oninput = (e) => form.zonePrice = e.target.value;
